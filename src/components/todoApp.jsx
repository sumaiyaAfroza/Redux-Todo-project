...
export const TodoApp = () => {
  ...
  const states = useSelector(selectTodosState)
     ...
     <div className='max-w-2xl mx-auto'>
       ...
       {states.total > 0 && (
         <div className='bg-white/90 backdrop-blur-sm rounded-2xl p-6 mb-6 border border-gray-300 shadow-lg'>
           ...
           <div className='w-full bg-gray-300 rounded-full h-3 mb-4'>
            ...
            transition-all duration-500 ease-out'  style={{width: `${states.completionPercentage}%`}}>
             </div>
           </div>
           <div className='grid grid-cols-3 gap-4 text-center'>
             <div>
               <div className='text-2xl font-bold text-gray-800'>{states.total}</div>
               <div className='text-sm text-gray-600'>Total</div>
             </div>
             <div>
               <div className='text-2xl font-bold text-gray-800'>{states.active}</div>
               <div className='text-sm text-gray-600'>Active</div>
             </div>
             <div>
               <div className='text-2xl font-bold text-gray-800'>{states.completed}</div>
       ...
       <div className='bg-white/90 backdrop-blur-sm rounded-b-2xl border border-gray-300 shadow-lg overflow-hidden'>
         <div className='p-6 border-b border-gray-300'>
           ...
           <TodoFilter states={states} currentfilter={filter} />
         </div>
           {
             isAddingTodo && (
